...
Environment kit (`world/kit/`): reusable prefabs spawned with `kit.spawn(name, x, z, yaw)`: walls (`wallStraight`, `wallShort`, `wallLow`, `wallCorner`, `wallEnd`, `wallBroken`), concrete (`barrierLarge`, `blockSmall`, `pillar`, `slabBroken`, `rubble`, `curb`), metal (`metalPanel`, `fence`, `metalBarricade`, `metalGate`) and wood (`crate`, `pallet`, `plankBarricade`). Parts are generated chamfered boxes with UVs in metres, so texel density is set per material (`KIT_SURFACES.*.tileMetres`) and never stretches; identical parts share cached meshes and everything is statically batched. `world/kit/TestLayout.ts` holds the small test composition around the spawn point.
