...
Weapons (`player/WeaponHolder.ts`, `WEAPONS` in config): six hand-held weapons from "assetpack-free" (assault rifle, shotgun, sniper rifle, pistol, knife, frag grenade), merged by `scripts/build-weapons.mjs` into `public/models/weapons/weapons.glb` (textures 2048 -> 512 WebP, 5.9 MB -> 1.5 MB). The chosen weapon (tune panel "Weapon", or `?weapon=<id|none>`) is parented to the hero's `mixamorig:RightHand` bone with a per-weapon grip (position, rotation, roll). Guns switch on an upper-body animation layer (masked from `mixamorig:Spine` up) playing the aiming clip `Run_and_Shoot`, so the arms hold the gun forward while the legs keep idling, walking or running; knife and grenade keep the normal arms.
