...
Weapons (`player/WeaponHolder.ts`, `WEAPONS` in config): 17 hand-held weapons merged by `scripts/build-weapons.mjs` into `public/models/weapons/weapons.glb` (~2.9 MB): six from "assetpack-free" (assault rifle, shotgun, sniper rifle, pistol, knife, frag grenade; textures 2048 -> 512 WebP), ten "Flat Guns East" guns (flat colours baked into vertex colours on one material, one draw call each) and a low poly axe by Yevhen Ishchenko (FBX converted with `scripts/fbx-to-glb.mjs`, 512 PBR maps). The chosen weapon (tune panel "Weapon", or `?weapon=<id|none>`) is parented to the hero's `mixamorig:RightHand` bone with a per-weapon grip (position, rotation, roll, optional scale). Weapons can declare `sockets` in their own space, `rightHandGrip` and `leftHandGrip` (a position plus the left hand's rotation), which become child entities of the held weapon (`WeaponHolder.socket(name)`). When the held weapon has a `leftHandGrip`, left-hand IK (`player/LeftHandIK.ts`, two-bone arm IK with a small collarbone assist, run every frame after the animation and blended by the upper-body layer weight) puts the left hand on it, so the clips stay untouched. The assault rifle uses it: pistol grip in the right palm, left hand under the rear of the handguard, muzzle turned 10° inward so the handguard stays within the Vanguard's short arm reach. Weapon grips and sockets are authored on the Vanguard's hands; a rig whose hand bones are rolled differently declares `handRollDeg` in `CHARACTERS` (the Survivor does) and the weapon holder and IK undo that roll, so all weapons fit it unchanged. On rigs with finger joints, `player/FingerGrip.ts` closes the right hand around any held weapon and the left hand on a `leftHandGrip` (`FINGER_GRIP` in config). Because the aiming clip is authored with the torso turned, an aim correction (`player/AimTwist.ts`, `AIM` in config) runs before the IK: it twists the three spine bones (up to 30°, smoothed) so the held gun points where the hero faces (within a few degrees while walking and running). Guns switch on an upper-body animation layer (masked from `mixamorig:Spine` up) playing the aiming clip `Run_and_Shoot`, so the arms hold the gun forward while the legs keep idling, walking or running. Melee weapons have an attack: Space or the on-screen ATTACK button plays `Axe_Spin_Attack` (axe) or `Attack` (knife) once on a full-body action layer.
