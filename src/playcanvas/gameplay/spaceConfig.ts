...
export type SpaceSkinId =
  | "skitter_green" | "skitter_violet" | "skitter_red"
  | "spitter_toxic" | "spitter_blue"
  | "glub_pink" | "glub_teal"
  | "brute_green" | "brute_crimson" | "brood_queen"
...
export const SPACE_SKINS: Record<SpaceSkinId, string> = {
  skitter_green: skin("skitter_green"), skitter_violet: skin("skitter_violet"), skitter_red: skin("skitter_red"),
  spitter_toxic: skin("spitter_toxic"), spitter_blue: skin("spitter_blue"),
  glub_pink: skin("glub_pink"), glub_teal: skin("glub_teal"),
  brute_green: skin("brute_green"), brute_crimson: skin("brute_crimson"), brood_queen: skin("brood_queen"),
...
export type SpaceEnemyId = "infected" | "sprinter" | "skitter" | "spitter" | "glub" | "hulk" | "secmech" | "warden" | "broodmother";
...
export const SPACE_ENEMIES: Record<SpaceEnemyId, EnemyDef> = {
  ...
  broodmother: {
    ...
    maxHp: 16000, speed: 2.0, radius: 1.5, damage: 32, attackRange: 3.3, attackWindup: 0.55, attackCooldown: 1.2, knockback: 8,
...
export const SPACE_BOSSES: Record<string, BossPhase[]> = {
  ...
  // The final exam on the meteor: melee, acid, the hive, the sky falling - then faster.
  broodmother: [
    {
      // PHASE 1 - THE QUEEN
    ...
    },
    {
      ...
      attacks: [
        ...
        { kind: "summon", enemies: [{ type: "hulk", count: 1 }, { type: "skitter", count: 6 }], radius: 5, portal: 1.2 },
...
/* ------------------------------------------------------------------------------------------------
 * The campaign: six levels in two acts (medium / hard / boss each), fought in the ship's zones
 * (world/level/ShipLevel.ts). These numbers are Hard; SPACE_DIFFICULTIES derive Easy / Nightmare.
 * ---------------------------------------------------------------------------------------------- */

export const SPACE_LEVELS: WaveDef[] = [
  {
    label: "ORION I", act: "Act I · Below Decks", subtitle: "The Cargo Bay", zone: "cargo",
    duration: 70, prespawn: 6, hpScale: 1, damageScale: 1,
    phases: [
      { at: 0, spawnInterval: 3.0, group: [2, 3], maxAlive: 10, weights: { infected: 1 } },
      { at: 20, spawnInterval: 2.6, group: [3, 4], maxAlive: 14, weights: { infected: 1, skitter: 0.5 } },
      { at: 45, spawnInterval: 2.2, group: [3, 5], maxAlive: 17, weights: { infected: 1, skitter: 0.7, sprinter: 0.2 } },
    ],
  },
  {
    label: "ORION II", act: "Act I · Below Decks", subtitle: "The Bio-Lab", zone: "biolab",
    duration: 85, prespawn: 8, hpScale: 1.15, damageScale: 1.1,
    phases: [
      { at: 0, spawnInterval: 2.6, group: [3, 4], maxAlive: 14, weights: { infected: 1, skitter: 0.8 } },
      { at: 20, spawnInterval: 2.4, group: [3, 5], maxAlive: 16, weights: { infected: 1, skitter: 0.9, spitter: 0.3, sprinter: 0.3 } },
      { at: 55, spawnInterval: 2.1, group: [4, 6], maxAlive: 19, weights: { infected: 1, skitter: 1, spitter: 0.4, sprinter: 0.4, hulk: 0.05 } },
    ],
  },
  {
    label: "ORION III", act: "Act I · Below Decks", subtitle: "The Reactor Core", zone: "reactor",
    duration: 40, prespawn: 6, hpScale: 1.25, damageScale: 1.15,
    phases: [
      { at: 0, spawnInterval: 2.6, group: [3, 4], maxAlive: 12, weights: { infected: 1, skitter: 0.8, spitter: 0.15 } },
    ],
    boss: { type: "warden", spawnFactor: 0.25 },
  },
  {
    label: "ORION IV", act: "Act II · Abandon Ship", subtitle: "The Command Deck", zone: "command",
    duration: 90, prespawn: 8, hpScale: 1.7, damageScale: 1.4, speedScale: 1.08,
    traps: { every: 13, kinds: ["tar", "sweep"], damage: 16 },
    phases: [
      { at: 0, spawnInterval: 2.4, group: [3, 5], maxAlive: 17, weights: { infected: 1, spitter: 0.5, glub: 0.2, sprinter: 0.4 } },
      { at: 30, spawnInterval: 2.1, group: [4, 6], maxAlive: 20, weights: { infected: 1, spitter: 0.6, glub: 0.3, sprinter: 0.5, skitter: 0.8, hulk: 0.08 } },
      { at: 65, spawnInterval: 1.8, group: [4, 6], maxAlive: 23, weights: { infected: 0.9, spitter: 0.7, glub: 0.35, sprinter: 0.6, skitter: 1, hulk: 0.12, secmech: 0.06 } },
    ],
  },
  {
    label: "ORION V", act: "Act II · Abandon Ship", subtitle: "The Hangar", zone: "hangar",
    duration: 105, prespawn: 10, hpScale: 2.4, damageScale: 1.8, speedScale: 1.14,
    traps: { every: 10, kinds: ["tar", "sweep", "cage", "cage"], damage: 20 },
    phases: [
      { at: 0, spawnInterval: 2.2, group: [4, 6], maxAlive: 20, weights: { infected: 1, spitter: 0.7, glub: 0.4, sprinter: 0.6, skitter: 1, hulk: 0.1 } },
      { at: 35, spawnInterval: 1.9, group: [4, 7], maxAlive: 24, weights: { infected: 0.9, spitter: 0.8, glub: 0.45, sprinter: 0.7, skitter: 1.1, hulk: 0.15, secmech: 0.1 } },
      { at: 75, spawnInterval: 1.6, group: [5, 7], maxAlive: 27, weights: { infected: 0.8, spitter: 0.9, glub: 0.5, sprinter: 0.8, skitter: 1.2, hulk: 0.2, secmech: 0.15 } },
    ],
  },
  {
    label: "ORION VI", act: "Act II · Abandon Ship", subtitle: "The Meteor", zone: "meteor",
    duration: 30, prespawn: 8, hpScale: 2.8, damageScale: 2.1, speedScale: 1.2, bossDamageScale: 1.6,
    traps: { every: 11, kinds: ["tar", "sweep", "cage"], damage: 22 },
    // The sky over the meteor never stops falling.
    meteors: { every: 8, damage: 24, radius: 2.3, count: 3 },
    phases: [
...
export const SPACE_RUN = {
  ...
  finalLevel: 5,
  /** After the Warden the ship's armory offers one of these free. */
  armory: { after: 2, weapons: ["plasma", "shotgun"] as const },
  text: {
    armory: ["THE SHIP'S ARMORY", "Past the Warden, the security locker stands open. Take one weapon - the other waits in the shop."] as [string, string],
    pact: ["EXPERIMENTAL AUGMENTS", "The medbay's prototypes could save you - or cost you. Choose one augment; its power has a price."] as [string, string],
    finalWarning: ["THE LAST AIRLOCK", "Beyond the airlock: the meteor the hive came from, and the Brood Mother. There is no ship to go back to."] as [string, string],
  },
};
...
export const SPACE_DIFFICULTIES: Record<"easy" | "hard" | "nightmare", DifficultyDef> = {
  easy: {
    ...
    levels: [
      ...
      { hpScale: 1.6, damageScale: 1.3, speedScale: 1, traps: undefined },
  ...
  nightmare: {
    label: "Nightmare",
    text: "Built to kill you. Maybe one run in a hundred leaves the meteor alive.",
    levels: [undefined, { traps: { every: 16, kinds: ["tar", "sweep"], damage: 12 } }, { traps: { every: 15, kinds: ["tar", "sweep"], damage: 14 } }],
    scale: {
      hp: [1.15, 1.2, 1.2, 1.3, 1.3, 1.4],
      damage: [1.1, 1.1, 1.15, 1.2, 1.2, 1.3],
      speed: [1.03, 1.03, 1.03, 1.07, 1.07, 1.07],
      bossHp: 1.3,
      bossDamage: 1.2,
      trapsEvery: 0.75,
      trapsDamage: 1.25,
